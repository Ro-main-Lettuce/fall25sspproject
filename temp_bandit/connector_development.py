...
@@ -29,6 +150,7 @@ def create_stream_template(connector_name, stream_name, url_base, path, http_met
...
+        _ = connector_name  # Context parameter for future use
         stream_template = {
             ...
             ""name"": stream_name,
@@ -40,138 +162,128 @@ def create_stream_template(connector_name, stream_name, url_base, path, http_met
                     ""url_base"": url_base,
                     ""path"": path,
...
+            if key not in RESERVED_STREAM_CONFIG_KEYS:
                 if ""."" in key:
                     keys = key.split(""."")
                     current = new_stream
@@ -180,81 +292,66 @@ def create_stream_from_template(connector_name, template_config, stream_config,
