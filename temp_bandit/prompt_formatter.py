...
+
 ...
 import json
 import logging
@@ -29,34 +31,46 @@ def _validate_messages(messages: list[dict]) -> None:
...
@@ -82,29 +96,24 @@ def create_generic_request(self, row: _DictOrBaseModel, idx: int) -> GenericRequ
             messages=messages,
             original_row=row,
...
@@ -121,10 +130,7 @@ def response_to_response_format(self, response_message: str | dict) -> Optional[
                 try:
...
@@ -141,17 +147,23 @@ def response_to_response_format(self, response_message: str | dict) -> Optional[
             )
             raise e
